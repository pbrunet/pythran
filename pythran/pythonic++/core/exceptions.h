...
namespace pythonic {
    namespace core {
        namespace BaseError{
            ...
            struct Type<0>
            {
                typedef core::list<std::string> type;
            };
        }
    ...
    class name:public parent \
        { \
            public: \
                ...
                name(std::string first, Types ... types):parent(first, types ...){} \
                virtual ~name() throw(){} \
        };
...
CLASS_EXCEPTION(UnicodeDecodeError,UnicodeError);
CLASS_EXCEPTION(UnicodeEncodeError,UnicodeError);
CLASS_EXCEPTION(UnicodeTranslateError,UnicodeError);
    }
}
